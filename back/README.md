...
Все эндпоинты доступны по базовому URL: `http://localhost:8080` (или любой другой порт, указанный в конфигурации).
...
1. **Регистрация:** 
   - Отправить `POST /auth/register` с именем и email
...
3. **Вход:**
   - Отправить `POST /auth/login` с email и паролем
   - Получить JWT токен
   - Сохранить токен для дальнейших запросов
